...
export default function (doctorRoleId) {
  describe('01. Create new file template', () => {
    const templateData = generateFileTemplateData(doctorRoleId)

    const createTemplateResponse = session.post(
      '/doctor/FileTemplate', JSON.stringify(templateData)
    )
    ...
    session.newFileTemplateId = createTemplateResponse.json('data.fileTemplateId')
    expect(session.newFileTemplateId, 'New template ID').to.be.a('string')
  })

  describe('02. Get patient template by ID', () => {
    session.addTag('name', 'UserItemURL')

    const url = `/doctor/FileTemplate?templateId=${session.newFileTemplateId}&doctorId=${doctorRoleId}`
  ...
  describe('03. Rename patient template by ID', () => {
    ...
    const payload = { 
      templateId: session.newFileTemplateId, 
      name: generateRandomString(10),
      doctorId: doctorRoleId 
    }

    const updateResponse = session.patch('/doctor/FileTemplate', JSON.stringify(payload))
  ...
  describe('04. Delete patient template by ID', () => {
    const payload = { templateId: session.newFileTemplateId, doctorId: doctorRoleId }
    const deleteResponse = session.delete('/doctor/FileTemplate', JSON.stringify(payload))
